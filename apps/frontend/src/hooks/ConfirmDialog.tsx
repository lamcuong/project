...
const ConfirmationDialogContext = React.createContext((info: ConfirmDialogType, handler: () => Promise<any>) => {})
...
export const ConfirmationDialogContextProvider = (props: any) => {
  ...
  const showDialog = useCallback((info: ConfirmDialogType, handler: () => Promise<boolean>) => {
    setInfo((prevState) => ({
      ...prevState,
      ...info
    }))
  ...
  return (
    <>
      <ConfirmationDialogContext.Provider value={showDialog}>{children}</ConfirmationDialogContext.Provider>
      <AlertDialog open={isOpen} onOpenChange={() => setIsOpen(false)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter className='gap-2 w-1/2 mx-auto sm:w-full'>
            ...
            <AlertDialogCancel type='button'>{info.cancelText}</AlertDialogCancel>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </>
